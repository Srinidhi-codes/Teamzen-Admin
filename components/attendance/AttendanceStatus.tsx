...
export function AttendanceStatus({
  ...
  return (
    <div className="group bg-card/60 backdrop-blur-md rounded-4xl p-6 border border-border shadow-xl shadow-primary/5 hover:shadow-primary/10 transition-all duration-500">

      <div className="flex items-center gap-4 mb-6">
        <div
          className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-500 ${config.color === 'emerald' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : config.color === 'destructive' ? 'bg-destructive/10 text-destructive' : config.color === 'amber' ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400' : 'bg-primary/10 text-primary'}`}
        >
        ...
        <div>
          ...
          <p className={`text-lg font-black tracking-tight ${config.color === 'emerald' ? 'text-emerald-600 dark:text-emerald-400' : config.color === 'destructive' ? 'text-destructive' : config.color === 'amber' ? 'text-amber-600 dark:text-amber-400' : 'text-primary'}`}>
            {config.label}
          </p>
        </div>
      </div>
