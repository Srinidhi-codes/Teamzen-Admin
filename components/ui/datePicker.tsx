        ...
        <div className={cn("flex flex-col space-y-2", className)}>
            ...
            <div className="sm:hidden">
                <input
                    type="date"
                    className={cn(
                        "w-full h-[35px] rounded-2xl bg-muted/5 border border-border/50 font-medium px-5 shadow-[inset_0_2px_4px_0_rgba(0,0,0,0.08)] focus:ring-4 focus:ring-primary/10 transition-all duration-300 outline-none appearance-none",
                        !displayDate && "text-muted-foreground/50",
                        error && "border-destructive/50"
                    )}
                    value={displayDate ? moment(displayDate).format("YYYY-MM-DD") : ""}
                    onChange={(e) => {
